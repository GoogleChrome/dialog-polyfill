var dialogPolyfill = (function() {
    ...
    dialogPolyfill.reposition = function(element) {
        var scrollTop = document.body.scrollTop || document.documentElement.scrollTop,
            windowHeight = ('undefined' == typeof window.innerHeight) && document.body && document.body.clientHeight ? document.body.clientHeight : window.innerHeight,
    ...
    dialogPolyfill.isInlinePositionSetByStylesheet = function(element) {
        for (var i = 0; i < document.styleSheets.length; ++i) {
            ...
            for (var j = 0; j < cssRules.length; ++j) {
                ...
                if (!selectedNodes || !dialogPolyfill.inNodeList(selectedNodes, element)) continue;
        ...
        if (isModal) {
            ...
            var findElementToFocus = function(root) {
                for (var i = 0; i < root.children.length; i++) {
                    ...
                    elem.nodeName == 'BUTTON' || elem.nodeName == 'INPUT' || elem.nodeName == 'KEYGEN' || elem.nodeName == 'SELECT' || elem.nodeName == 'TEXTAREA')) {
    ...
    dialogPolyfill.close = function(retval) {
        if (!this.open) throw 'InvalidStateError: close called on closed dialog';
        this.open = false;
    ...
    dialogPolyfill.registerDialog = function(element) {
        ...
        element.show = function() {
            dialogPolyfill.showDialog.call(element, false);
        }
        element.showModal = function() {
            dialogPolyfill.showDialog.call(element, true);
        }
        element.close = function(retval) {
            dialogPolyfill.close.call(element, retval);
        }
    ...
    dialogPolyfill.DialogManager.prototype.updateStacking = function() {
        ...
        for (var i = 0; i < this.pendingDialogStack.length; i++) {
            if (i == this.pendingDialogStack.length - 1) this.overlay.style.zIndex = zIndex++;
    ...
    dialogPolyfill.DialogManager.prototype.handleKey = function(event) {
        ...
        if (event.keyCode == 9 /* tab */ ) {
            ...
            if (forward) {
                if (activeElement == document.documentElement || activeElement == document.body || activeElement == dialog.dialogPolyfillInfo.backdrop) {
    ...
    addEventListenerFn(document, 'keydown', function(ev) {
        dialogPolyfill.dm.handleKey.call(dialogPolyfill.dm, ev)
    });
